...
class Mesos(AgentCheck):
    ...
    def check(self, instance):
        ...
        # Load values from the instance config, grabbing the master
        url = self.get_master(instance['url'], timeout)
        ...
        if response is not None:
            tags = instance_tags
            for attr in ['activated_slaves', 'deactivated_slaves','failed_tasks','finished_tasks','killed_tasks','lost_tasks','staged_tasks','started_tasks']:
    ...
    def get_master_stats(self, url, timeout):
        return self.get_json(url + "/metrics/snapshot", timeout)
